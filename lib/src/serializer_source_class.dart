...
abstract class SerializerSourceClass
  ...
  String get wireName {
    if (isBuiltValue) {
      return builtValueSettings.wireName ?? fullName;
    } else if (isEnumClass) {
      return enumClassSettings.wireName ?? fullName;
    } else {
      return fullName;
    }
  }
  ...
  String _generateFieldDeserializers() {
    return fields.map((field) {
      ...
      if (field.builderFieldUsesNestedBuilder) {
        // Deserialization should not fail because this field is null.
        // Auto creating nested builders is convenient, but also not ideal
        // for a large object graph like one found in a Redux app.
        // Serializers has the 'newBuilder(FullType)' method which makes this
        // a trivial addition.
        if (!field.settings.autoCreateNestedBuilders) {
          return '''
case '${escapeString(field.wireName)}':
  if (result.${field.name} == null) result.${field.name} = serializers.newBuilder($fullType);
